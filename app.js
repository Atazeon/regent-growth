...
const outboundLaunchLogNote = document.querySelector("#outboundLaunchLogNote");
const outboundLaunchLogSummary = document.querySelector("#outboundLaunchLogSummary");
const outboundLaunchLogList = document.querySelector("#outboundLaunchLogList");
...
function normalizeOutboundLaunchLog(entry) {
  return {
    id: entry.id || createId(),
    status: entry.status || "Dry Run",
...
function renderOutboundLaunchLog() {
  const entries = outboundSessionState.launchLog || [];
  outboundLaunchLogSummary.textContent = entries.length
    ? `${entries.length} launch decision${entries.length === 1 ? "" : "s"} logged`
    : "No launch decisions logged yet.";
  [copyOutboundLaunchLogButton, downloadOutboundLaunchLogButton, clearOutboundLaunchLogButton].forEach((button) => {
    button.disabled = entries.length === 0;
  });
  outboundLaunchLogList.innerHTML = entries.length
    ? entries.map((entry) => `
      <article>
        <div>
          <p class="eyebrow">${escapeHtml(entry.status)}</p>
          <h4>${escapeHtml(entry.company || "Unassigned batch")}</h4>
          <p>${escapeHtml(entry.note)}</p>
          <small>${escapeHtml(formatDateTime(entry.createdAt))}</small>
        </div>
        <button class="secondary-button" type="button" data-action="delete-outbound-launch-log" data-id="${escapeHtml(entry.id)}">Remove</button>
      </article>
    `).join("")
    ...
    ? visibleSnapshots.slice(0, 5).map((snapshot, index) => `
      <article>
        ...
        <div class="outbound-run-snapshot-actions">
          ...
          <button class="secondary-button" type="button" data-action="note-outbound-run-snapshot" data-id="${escapeHtml(snapshot.id)}">Note</button>
...
function downloadOutboundRunSnapshots() {
  const snapshots = outboundSessionState.runSnapshots || [];
  if (snapshots.length === 0) {
    setDataStatus("No first real outbound run snapshots to download.", "error");
    return;
  }
...
function downloadOutboundRunSnapshotsCsv() {
  const snapshots = getVisibleOutboundRunSnapshots(outboundSessionState.runSnapshots || []);
  if (snapshots.length === 0) {
    setDataStatus("No visible first real outbound run snapshots to export as CSV.", "error");
    return;
  }

  const headers = ["label", "note", "savedAt", "state", "completedSteps", "totalSteps", "outcomes", "openFixes", "resolvedFixes", "archivedFixes"];
  const rows = snapshots.map((snapshot) => {
    ...
    return [
      ...
      readiness.openFixCount || 0,
      readiness.resolvedFixCount || 0,
      readiness.archivedFixCount || 0
    ];
  });
  const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
...
function restoreOutboundRunSnapshot(id) {
  ...
  outboundSessionState = {
    ...
    launchLog: Array.isArray(snapshot.launchLog)
      ? snapshot.launchLog.map(normalizeOutboundLaunchLog).filter((entry) => entry.status && entry.note)
...
function updateOutboundRunSnapshotNote(id) {
  const snapshots = outboundSessionState.runSnapshots || [];
  const snapshot = snapshots.find((item) => item.id === id);
  if (!snapshot) {
    setDataStatus("Saved first real outbound run snapshot was not found.", "error");
  ...
  const note = prompt("Snapshot note", snapshot.note || "");
  if (note === null) {
    setDataStatus("Snapshot note canceled.");
    return;
  }
  ...
  saveOutboundSessionState();
  renderOutboundSession();
  setDataStatus(snapshot.note ? "Saved first real outbound run snapshot note." : "Cleared first real outbound run snapshot note.");
}
...
function handleOutboundRunSnapshotListClick(event) {
  ...
  const noteButton = event.target.closest('[data-action="note-outbound-run-snapshot"]');
  if (noteButton) {
    updateOutboundRunSnapshotNote(noteButton.dataset.id);
    return;
  }
...
function getOutcomeCountsForBatch(batch) {
  return outboundSessionState.outcomes
    .filter((outcome) => (outcome.batch || "First Run") === batch)
...
function formatBatchComparison() {
  const firstCounts = getOutcomeCountsForBatch("First Run");
  const secondCounts = getOutcomeCountsForBatch("Second Batch");
  const comparisonRows = outboundOutcomeTypes.map((type) => {
    const first = firstCounts[type] || 0;
    const second = secondCounts[type] || 0;
    return `${type}: first ${first} | second ${second} | delta ${formatSignedDelta(second - first)}`;
  });
  const firstTotal = outboundSessionState.outcomes.filter((outcome) => (outcome.batch || "First Run") === "First Run").length;
  const secondTotal = getSecondBatchOutcomes().length;
  return [
    "First vs Second Outbound Batch Comparison",
    `Created: ${formatDateTime(new Date().toISOString())}`,
    `First-run outcomes: ${firstTotal}`,
    `Second-batch outcomes: ${secondTotal}`,
    "",
    "Outcome Deltas",
    ...comparisonRows,
    "",
    "Read",
    secondTotal === 0
      ? "No second-batch outcomes have been logged yet."
      : secondTotal > firstTotal
        ? "Second batch has more logged activity than the first run."
        : "Second batch is still equal to or smaller than the first run."
  ].join("\n");
}

async function copyBatchComparison() {
  const copiedDirectly = await copyTextWithFallback(formatBatchComparison());
  setDataStatus(copiedDirectly ? "Copied first vs second outbound batch comparison." : "Selected and copied first vs second outbound batch comparison.");
}

function downloadBatchComparison() {
  const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
...
function addOutboundLaunchLog(event) {
  event.preventDefault();
  const note = outboundLaunchLogNote.value.trim();
  if (!note) {
    setDataStatus("Add a launch decision note before saving.", "error");
    return;
  }

  outboundSessionState.launchLog = [
    normalizeOutboundLaunchLog({
      id: createId(),
      status: outboundLaunchLogStatus.value,
      company: outboundLaunchLogCompany.value.trim(),
      note,
      createdAt: new Date().toISOString()
    }),
    ...(outboundSessionState.launchLog || [])
  ];
  outboundLaunchLogCompany.value = "";
  outboundLaunchLogNote.value = "";
  saveOutboundSessionState();
  renderOutboundSession();
  setDataStatus("Logged first real outbound launch decision.");
}

function deleteOutboundLaunchLog(id) {
  const beforeCount = (outboundSessionState.launchLog || []).length;
  outboundSessionState.launchLog = (outboundSessionState.launchLog || []).filter((entry) => entry.id !== id);
  if (outboundSessionState.launchLog.length === beforeCount) return;
  saveOutboundSessionState();
  renderOutboundSession();
  setDataStatus("Removed first real outbound launch log entry.");
}

function formatOutboundLaunchLog() {
  const entries = outboundSessionState.launchLog || [];
  if (entries.length === 0) return "No first real outbound launch decisions logged yet.";

  return [
    "First Real Outbound Manual Launch Log",
    `Logged: ${entries.length}`,
    "",
    ...entries.map((entry) => [
      `${formatDateTime(entry.createdAt)} | ${entry.status} | ${entry.company || "Unassigned batch"}`,
      entry.note
    ].join("\n"))
  ].join("\n");
}

async function copyOutboundLaunchLog() {
  if (!(outboundSessionState.launchLog || []).length) {
    setDataStatus("No first real outbound launch log to copy.", "error");
    return;
  }

  const copiedDirectly = await copyTextWithFallback(formatOutboundLaunchLog());
  setDataStatus(copiedDirectly ? "Copied first real outbound launch log." : "Selected and copied first real outbound launch log.");
}

function downloadOutboundLaunchLog() {
  if (!(outboundSessionState.launchLog || []).length) {
    setDataStatus("No first real outbound launch log to download.", "error");
    return;
  }

  const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
  downloadFile(`regent-growth-first-run-launch-log-${stamp}.txt`, formatOutboundLaunchLog(), "text/plain;charset=utf-8");
  setDataStatus("Downloaded first real outbound launch log.");
}

function clearOutboundLaunchLog() {
  const count = (outboundSessionState.launchLog || []).length;
  if (!count) {
    setDataStatus("No first real outbound launch log to clear.", "error");
    return;
  }
  if (!confirm(`Clear ${count} first real outbound launch log entr${count === 1 ? "y" : "ies"}?`)) return;
  outboundSessionState.launchLog = [];
  saveOutboundSessionState();
  renderOutboundSession();
...
function formatOpenOutboundImprovementSummary() {
  const openItems = getOutboundImprovementItems().filter((item) => item.status === "Open");
  if (openItems.length === 0) return "No open outcome-driven product fixes queued.";

  return [
    "Open Outcome-Driven Product Fixes",
...
function formatFilteredOutboundImprovementCloseout() {
  const items = getVisibleOutboundImprovementItems().filter((item) => ["Resolved", "Archived"].includes(item.status));
  if (items.length === 0) return "No visible resolved or archived outcome-driven product fixes.";

  const statusLabel = outboundImprovementStatusFilterValue === "all" ? "All statuses" : outboundImprovementStatusFilterValue;
  const ownerLabel = outboundImprovementOwnerFilterValue === "all" ? "All owners" : outboundImprovementOwnerFilterValue;

  return [
    "Filtered Fix Closeout",
    `Status: ${statusLabel}`,
    `Owner: ${ownerLabel}`,
    `Closeout fixes: ${items.length}`,
    "",
    ...items.map((item) => [
      `${item.status} - ${item.type}`,
      `Company: ${item.company || "Unassigned company"}`,
...
async function copyOpenOutboundImprovementSummary() {
  const openItems = getOutboundImprovementItems().filter((item) => item.status === "Open");
  if (openItems.length === 0) {
    setDataStatus("No open outcome-driven fixes to copy.", "error");
    return;
  }
...
async function copyFilteredOutboundImprovementCloseout() {
  const items = getVisibleOutboundImprovementItems().filter((item) => ["Resolved", "Archived"].includes(item.status));
  if (items.length === 0) {
    setDataStatus("No visible resolved or archived fixes to close out.", "error");
    return;
  }
...
function renderOwnerDashboard() {
  ...
  ownerDashboardCount.textContent = totalHandoffs === 0
    ? "No active handoffs"
    : `${totalHandoffs} active | ${workloads.length} owner${workloads.length === 1 ? "" : "s"} | ${overdueHandoffs} overdue | ${blockedHandoffs} blocked | ${acceptedHandoffs} accepted`;
...
outboundLaunchLogForm.addEventListener("submit", addOutboundLaunchLog);
outboundLaunchLogList.addEventListener("click", (event) => {
  const deleteButton = event.target.closest('[data-action="delete-outbound-launch-log"]');
  if (deleteButton) deleteOutboundLaunchLog(deleteButton.dataset.id);
});
